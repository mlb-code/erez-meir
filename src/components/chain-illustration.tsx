    ...
    <div className="relative mx-auto aspect-square w-full max-w-md">
      ...
      {NODES.map((node) => (
        <div
          ...
          className="absolute w-28 -translate-x-1/2 -translate-y-1/2 rounded-card border border-ink-200 bg-white p-2.5 text-center shadow-md sm:w-32"
        >
          <span className="mx-auto mb-1 flex h-7 w-7 items-center justify-center rounded-chip bg-brand-100">
            <svg viewBox="0 0 24 24" className="h-4 w-4 text-brand-700" fill="currentColor">
              <path d="M12 3l9 8h-3v9h-5v-6h-2v6H6v-9H3z" />
            </svg>
          </span>
          <p className="text-caption font-extrabold text-ink-900">{node.name}</p>
      ...
      {CASH.map((label) => (
        <span
          ...
          className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-chip px-2 py-1 text-[11px] font-bold whitespace-nowrap shadow-sm ${
